...
class ClockDisplay extends React.Component {
    ...
    toggleClock = () => {
      let clockState = this.state.showClock ? false : true
      this.setState({showClock: clockState})
    }
    ...
    render() {
        return (
            <div className="ClockDisplay componentBox">
                ...
                <button onClick={this.toggleClock}>{this.state.showClock ? "Hide " : "Show "}Clock</button>
            </div>
        )
    }
}
