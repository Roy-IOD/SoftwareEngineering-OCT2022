...
class ClockDisplay extends React.Component {
    ...
    //   let toggle =  this.state.showClock === true ? false : true
    //   this.setState({showClock: toggle}) 
    }
    ...
    render() {
        return (
            <div className="ClockDisplay componentBox">
                ...
                <button onClick={this.toggleClock}>{this.state.showClock ? 'Hide' : 'Show'}</button> {/* update this button to use the new toggleClock function and say 'Hide' or 'Show' depending */}
            </div>
        )
    }
}
