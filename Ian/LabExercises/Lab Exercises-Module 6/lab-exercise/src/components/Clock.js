...
class ClockDisplay extends React.Component {
    ...
    toggleClock = () => {
        let showHide = this.state.showClock === true ? false : true;
        this.setState({ showClock : showHide})
    }
    ...
    render() {
        return (
            <div className="ClockDisplay componentBox">
                ...
                <button onClick={this.toggleClock}>{this.state.showClock ? 'Hide' : 'Show'} Clock</button> {/* update this button to use the new toggleClock function and say 'Hide' or 'Show' depending */}
            </div>
        )
    }
}
