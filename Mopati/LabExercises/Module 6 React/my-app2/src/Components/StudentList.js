...
function Student(props) {
  ...
  const avatarSize = props.avatarSize
    ? props.avatarSize + "x" + props.avatarSize
    : "50x50";
  ...
  return (
    <li>
      <div className="avatar">
        <img
          src={`https://robohash.org/${name}?size=${avatarSize}&set=${roboset}`}
          alt={name}
        ></img>
      </div>
      <div className="details">
        <strong>{name}</strong>
        <span className="location">based in{props.student.location}</span>
      </div>
    </li>
  );
}
