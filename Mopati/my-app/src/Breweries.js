...
class Breweries extends Component {
  ...
  getBreweries = (page, filterType) => {
    ...
    fetch(
      "https://api.openbrewerydb.org/breweries?per_page=50&page=" +
        page +
        filterParam
    )
    ...
    return (
      <div className="Breweries componentBox">
        ...
        <TypeFilter
          ...
          filterBreweryHandler={this.getBreweries}
        />

        <ul className="hideBullets">
          {this.state.breweries.map(
            (
              brewery //try also moving the below out into a separate Brewery component
            ) => (
              <li key={brewery.id}>
                <a
                  target="_blank"
                  href={`https://api.openbrewerydb.org/breweries/${brewery.id}`}
                >
                  {brewery.name}
                </a>{" "}
                is a {brewery.brewery_type} brewery in {brewery.state},{" "}
                {brewery.country}
              </li>
            )
          )}
        ...
        <Pagination
          currentPage={this.state.currentPage}
          changePageHandler={this.handleChangePage}
        />
      </div>
    );
  }
  ...
  return (
    <div className="TypeFilter">
      <select
        ...
        onChange={(e) =>
          props.filterBreweryHandler(props.currentPage, e.target.value)
        }
      >
